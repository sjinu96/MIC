...
class XTransformer(BasicModel):
    ...
    def forward(self, **kwargs):
        ...
        #print("2 att_feats.shape:", att_feats.shape) # [8,18,2048]
        #print("2 att_feats:", att_feats)
        ...
        ##############################################
        # print('seq_mask : ', seq_mask.shape) # [B, 90, 90]
        # print('att_feats.shape', att_feats.shape) # [B, input_seq, attn_dim]

        # print('att_feats_shape', att_feats.shape)
        # print('att_mask ', att_mask.shape)
        # print(att_feats.shap)
        # print('1', att_feats.shape)
        # print(att_feats.stride()) # (514, 1024, 1)
        # att_feats = self.att_embed(att_feats)
        # print(att_feats.contiguous().stride())
        # print(att_feats.stride()) # (200704, 512, 121)
        # print('2', att_feats.shape)
        

        # 1-31 : (아마) Concat + CA를 이용할 때 contiguous 문제가 발생하는 것으로 보임(BiP 진행할 때 tensor.view를 사용 못함.)

        att_feats = att_feats.contiguous()
        # print('ㅠㅠ')

        # print('att_feats.shape', att_feats.shape) # [B, input_seq, embed_dim]
        #print("3 att_feats.shape:", att_feats.shape) # [8,18,768]
        #print("3 att_feats:", att_feats)
        # print(att_feats.shape, att_mask.shape)
    ...
    def decode_beam(self, **kwargs):
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            selected_beam = selected_idx // candidate_logprob.shape[-1] # changed
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        #print("mask.unsqueeze(-1).shape:", mask.unsqueeze(-1).shape) # [B, 1029, 1]
        #print("mask.unsqueeze(-1):", mask.unsqueeze(-1)) # 
        gx = (torch.sum(x * mask.unsqueeze(-1), 1) / torch.sum(mask.unsqueeze(-1), 1)) # [8,768]

        gx_arr = [gx]
        for layer in self.layers:
            #print("gx.shape:", gx.shape) # [B, 256]
            #print("x.shape:", x.shape)   # [B, 1029, 256]
            #print("mask.shape:", mask.shape) # [B, 1029]
